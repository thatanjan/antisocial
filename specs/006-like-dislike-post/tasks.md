...
**Goal**: Implement the core "Like" toggle functionality with optimistic UI.
...
- [ ] T006 [P] [US1] Create `LikeButton.tsx` skeleton in `src/features/likes/components/LikeButton.tsx`
- [ ] T007 [US1] Implement `useOptimistic` hook in `LikeButton.tsx` to handle heart icon toggle and count
- [ ] T008 [US1] Update post fetching logic (likely in `src/app/(authenticated)/page.tsx` or similar server component) to include `isLiked` and `likeCount` in post data
- [ ] T009 [US1] Integrate `LikeButton` into `src/features/create-post/components/PostCard.tsx` replacing the current Heart placeholder
...
- [ ] T010 [US2] Update `LikeButton.tsx` to handle Promise rejection from `toggleLikeAction`
- [ ] T011 [US2] Add toast notification on failure in `LikeButton.tsx` using `useToast` from Shadcn UI
...
- [ ] T012 Add TSDoc comments to all exports in `src/features/likes/`
- [ ] T013 Ensure `LikeButton` has appropriate `aria-label` for accessibility
- [ ] T014 Run validation per `quickstart.md` manual verification steps
